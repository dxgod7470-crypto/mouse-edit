...
import com.sazanx.mouseconfigurator.optimization.*
...
class MainActivity : Activity() {
    ...
    private fun updateShizukuStatus() {
        if (::status.isInitialized) status.text = shizuku.statusText()
    }
    ...
    private fun buildUi() {
        ...
        val actions = listOf({ showConfig() }, { showTest() }, { showDevices() }, { showOptimization() })
        buttons.indices.forEach { i ->
            tabs.addView(Button(this).apply {
                text = buttons[i]
                setOnClickListener { actions[i].invoke() }
            }, LinearLayout.LayoutParams(0, -2, 1f))
        }
        root.addView(tabs)
        ...
        fun configPage() = show {
            ...
            listOf("Linear", "Soft", "Windows-like", "Aggressive").forEach { name ->
                rg.addView(RadioButton(this).apply {
                    text = name; setTextColor(Color.WHITE); isChecked = name == cfg.curve
        ...
        fun testPage() = show {
            ...
            log = TextView(this).apply {
                ...
                setTextColor(Color.rgb(180, 220, 255)); textSize = 12f
            }
            body.addView(log)
        }
        fun devicePage() = show {
        ...
        fun optimizationPage() = show {
            ...
            addText(body, "These controls reduce this app's own workload. They cannot increase the physical speed of Wi‑Fi/mobile data or force CPU/GPU frequencies.")
            ...
            addSwitch(body, "Adaptive processing", current.adaptiveProcessing) {
                // Kept in the UI for the next service-level adaptive policy.
            }
            ...
            addText(body, "Recommended for low hardware usage: Raw input + Low Latency profile + adaptive/thermal protection.")
            startStatsTimer()
        }

        fun setPage(index: Int) {
            when (index) { 0 -> configPage(); 1 -> testPage(); 2 -> devicePage(); else -> optimizationPage() }
        }
        buttons.indices.forEach { i -> tabs.getChildAt(i).setOnClickListener { setPage(i) } }
        showConfig = { configPage() }
        showTest = { testPage() }
        showDevices = { devicePage() }
        showOptimization = { optimizationPage() }
        configPage()
    }

    private var showConfig: () -> Unit = {}
    private var showTest: () -> Unit = {}
    private var showDevices: () -> Unit = {}
    private var showOptimization: () -> Unit = {}
    ...
    private fun addText(p: LinearLayout, s: String) {
        p.addView(TextView(this).apply {
            text = s; setTextColor(Color.LTGRAY); textSize = 13f; setPadding(0, 6, 0, 6)
        })
    }
    ...
    private fun addSwitch(p: LinearLayout, label: String, initial: Boolean, change: (Boolean) -> Unit) {
        p.addView(Switch(this).apply {
            text = label; setTextColor(Color.WHITE); isChecked = initial
    ...
    private fun startMouseService() {
        if (!shizuku.isRunning()) {
            Toast.makeText(this, "Start Shizuku first.", Toast.LENGTH_SHORT).show(); return
        }
    ...
    override fun dispatchGenericMotionEvent(e: MotionEvent): Boolean {
        if ((e.source and InputDevice.SOURCE_MOUSE) == InputDevice.SOURCE_MOUSE) {
            ...
            val rawX = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                e.getAxisValue(MotionEvent.AXIS_RELATIVE_X) else e.getAxisValue(MotionEvent.AXIS_X)
            val rawY = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                e.getAxisValue(MotionEvent.AXIS_RELATIVE_Y) else e.getAxisValue(MotionEvent.AXIS_Y)
